...
// Modele Menu
class Menu
  extends Model<MenuAttributes, Optional<MenuAttributes, "menu_id">>
...
Menu.init(
  {
    menu_id: { type: DataTypes.STRING, primaryKey: true, allowNull: true },
...
const generateCustomMenuId = async (): Promise<string> => {
  ...
  if (lastMenu && lastMenu.menu_id) {
    const lastId = parseInt(lastMenu.menu_id.slice(3), 10);
    nextIdNumber = lastId + 1;
  }

  const paddedNumber = String(nextIdNumber).padStart(6, "0");
  return `MEN${paddedNumber}`;
};

Menu.beforeCreate(async (menu) => {
  menu.menu_id = await generateCustomMenuId();
});
...
class Menu_Item
  extends Model<
    Menu_ItemAttributes,
    Optional<Menu_ItemAttributes, "menu_id" | "item_id">
  >
  implements Menu_ItemAttributes
{
  public menu_id!: string;
